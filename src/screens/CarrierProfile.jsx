...
const CarrierProfile = ({ navigation, route }) => {
  ...
  const [flightUserData, setFlightUserData] = useState({});

  const getUserData = async () => {
    setLoading(true)
    let token = await AsyncStorage.getItem('usertoken');
    await axios
      .get(`${BASE_API_URI}/get/user?id=${route?.params?.userId}`, {
        headers: { Authorization: `Bearer ${token}` },
      })
      .then(responseData => {
        setLoading(false)
        console.log(responseData.data,"responseData")
        setFlightUserData(responseData.data?.[0]);
      }).catch((err)=>{
        setLoading(false)
      })
  };
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.profileCard}>
          ...
          <Text style={styles.profileName}>{flightUserData?.name?flightUserData?.name?.split("/")?.join(" "):"...."}</Text>
          <Text style={styles.verifiedText}>
            <Text style={{ color: '#10b981' }}>Verified ID</Text> • Frequent
            Flyer
          </Text>
          ...
          <View style={styles.statsRow}>
            ...
            <View style={styles.statBox}>
              <Text style={styles.statValue}>
                <Ionicons name="airplane" color="#1E90FF" size={16} />{' '}
                {profile.trips}
              </Text>
        ...
        <View style={styles.sectionCard}>
          <View style={styles.sectionHeader}>
            ...
            <View style={styles.flightBadge}>
              <Text style={styles.flightBadgeText}>{route?.params?.data?.airline_name}</Text>
            </View>
          </View>
          ...
          <View style={styles.timelineContainer}>
            ...
            <View style={styles.timelineRight}>
              <View style={styles.timelinePoint}>
                <Text style={styles.airportCode}>{route?.params?.data?.departure}</Text>
                {/* <Text style={styles.locationSub}>New York, USA</Text> */}
                <Text style={styles.dateTime}>{moment(route?.params?.data?.travel_date).calendar()}{' '}{route?.params?.data?.departure_time}</Text>
              </View>
              <View style={[styles.timelinePoint, { marginTop: 30 }]}>
                <Text style={styles.airportCode}>{route?.params?.data?.destination}</Text>
                {/* <Text style={styles.locationSub}>London, UK</Text> */}
                <Text style={styles.dateTime}>{moment(route?.params?.data?.travel_date).calendar()}{' '}{route?.params?.data?.arrival_time}</Text>
              </View>
            </View>
          </View>
        </View>
        ...
        <View style={styles.sectionCard}>
          ...
          <View style={styles.capacityRow}>
            ...
            <Text style={styles.capacityValue}>
              <Text style={{ color: '#fff' }}>{route?.params?.data?.capicity_in_kg-((route?.params?.data?.already_full_capicity_in_grams)*1000)}kg</Text> /{' '}
              {route?.params?.data?.capicity_in_kg}kg
            </Text>
          </View>
          <View style={styles.progressBarBg}>
            <View
              style={[
                ...
                {
                  width: `${
                    ((route?.params?.data?.capicity_in_kg-((route?.params?.data?.already_full_capicity_in_grams)*1000)) / route?.params?.data?.capicity_in_kg) * 100
                  }%`,
                },
              ]}
            />
        ...
        <View style={styles.preferencesGrid}>
          {profile.preferences.map(pref => (
            <View
              ...
              style={[
                styles.prefItem,
                { backgroundColor: pref.allowed ? '#1e293b' : '#2d1e23' },
              ]}
            >
              <Ionicons
      ...
      <View style={styles.stickyFooter}>
        ...
        <TouchableOpacity
          ...
          onPress={() => navigation.navigate('NewParcelRequest',{fid:route?.params?.data?._id})}
        >
