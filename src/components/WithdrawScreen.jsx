...
const WithdrawScreen = () => {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      Alert.alert('Unable to load withdrawal details', error?.response?.data?.message || error.message || 'Please try again.');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const submit = async () => {
    if (!payoutsEnabled) {
      Alert.alert('Bank setup required', 'Complete your Stripe bank setup before requesting a withdrawal.');
      return;
    }
    ...
    if (requestedAmount > wallet.availableBalance) {
      Alert.alert('Insufficient balance', `You can withdraw up to ${money(wallet.availableBalance)}.`);
      return;
    }
    try {
      setSubmitting(true);
      ...
      setAmount('');
      Alert.alert('Request submitted', 'Your withdrawal request is now pending admin approval.');
      loadData();
    } catch (error) {
      Alert.alert('Unable to request withdrawal', error?.response?.data?.message || error.message || 'Please try again.');
    } finally {
      setSubmitting(false);
    }
  };
...
const styles = StyleSheet.create({
  ...
  requestButton: { height: 55, borderRadius: 12, backgroundColor: '#2563eb', marginTop: 16, justifyContent: 'center', alignItems: 'center' }, disabled: { opacity: .5 }, requestText: { color: '#fff', fontSize: 16, fontWeight: '700' },
